...
#include "../../blocks/optix/optix_stack_size.h"
...
#define OPTIX_CHECK(callfun)                                                \
  do {                                                                      \
    ...
    if (ret != OPTIX_SUCCESS) {                                             \
      CI_LOG_E(__FILE__ << ":" << __LINE__ << " Optix call" << #callfun \
                << " failed. retcode " << ret << "");                      \
      exit(-1);                                                             \
    }                                                                       \
  } while (0)
...
static void BuildAccel(const OptixDeviceContext& context,
    ...
    CUdeviceptr d_gas_output_buffer;
    {
        ...
        CI_LOG_I("vertices_size: " << vertices_size << "");
    ...
    CI_LOG_I("gas_handle: " << gas_handle << "");
...
int main(int argc, char** argv) {
    ...
    const std::string ptx_filename = app::getAssetPath("optixTriangle.ptx").string();
    ...
    if (cuewInit(CUEW_INIT_CUDA) != CUEW_SUCCESS) {
        CI_LOG_E("Failed to initialize CUDA");
        return -1;
    }
    ...
    if (cuewInit(CUEW_INIT_NVRTC) != CUEW_SUCCESS) {
        CI_LOG_E("Failed to initialize NVRTC. NVRTC library is not available "
        ...
        CI_LOG_I("Found NVRTC runtime compilation library version " << major
            << "." << minor << "");
    }

    if (cuInit(0) != CUDA_SUCCESS) {
        CI_LOG_E("Failed to init CUDA");
        return -1;
    }
    ...
    CUcontext cuCtx{};
    {
        ...
        CI_LOG_I("# of CUDA devices: " << counts << "");
        if (counts < 1) {
            CI_LOG_E("No CUDA device found");
            return -1;
        }

        CUdevice device{};
        if (CUDA_SUCCESS != cuDeviceGet(&device, /* devid */ 0)) {
            CI_LOG_E("Failed to get CUDA device.");
            return -1;
        }

        {
            ...
            CI_LOG_I("compute capability: " << major << "." << minor << "");
        }

        if (CUDA_SUCCESS != cuCtxCreate(&cuCtx, /* flags */ 0, device)) {
            CI_LOG_E("Failed to get Create CUDA context.");
            return -1;
        }
    }
...

    {
        if (optixInit() != OPTIX_SUCCESS) {
            CI_LOG_E("Failed to initialize OptiX");
            return -1;
        }
        ...
        OptixPipelineCompileOptions pipeline_compile_options = {};
        {
            ...
            if (!LoadPTXFromFile(ptx_filename, &ptx)) {
                CI_LOG_E("Failed to open/read PTX file: " << ptx_filename << "");
                exit(-1);
            }
        ...
        OptixPipeline pipeline = nullptr;
        {
            ...
            CI_LOG_I("trav: " << direct_callable_stack_size_from_traversal
        ...
        OptixShaderBindingTable sbt = {};
        {
            ...
            // CI_LOG_I("hitGroupSz " << sizeof(HitGroupSbtRecord) << "");
        }
        ...
        Params params;
        {
            ...
            CI_LOG_I("offsetof(image_width) = " << offsetof(Params, image_width) << "");
            CI_LOG_I("offsetof(image_height) = " << offsetof(Params, image_height) << "");
            CI_LOG_I("offsetof(cam_u) = " << offsetof(Params, cam_u) << "");
            CI_LOG_I("offsetof(cam_v) = " << offsetof(Params, cam_v) << "");
            CI_LOG_I("offsetof(cam_w) = " << offsetof(Params, cam_w) << "");
            CI_LOG_I("offsetof(handle) = " << offsetof(Params, handle) << "");
...
#if 0
        {
            ...
            if (n < 1) {
                CI_LOG_E("Failed to write PNG image.");
                exit(-1);
            }
        }
#endif
...
struct MiniOptixApp : public App
{
    void setup() override
    {
...
CINDER_APP(MiniOptixApp, RendererGl, [](App::Settings* settings) {
    readConfig();
